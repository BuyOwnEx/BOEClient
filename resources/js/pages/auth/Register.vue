<template>
	<div class='layout-content ma-auto'>
		<v-card>
				...
				<v-container class='pt-0 pb-0'>
					<v-row>
						...
						<v-col cols='12' md='12' class='pt-0 pb-0'>
							...
							>
								<template #label>
									Confirm password <span class='red--text'><strong>*</strong></span>
								</template>
							</v-text-field>
						</v-col>
					</v-row>
			...
			<div class='text-left pl-6 pr-6'><small><span class='red--text'><strong>*</strong></span> indicates required
				field</small></div>
			<v-card-actions class='pt-4 pl-6 pr-6'>
				<v-btn block color='primary' style='border-radius: unset' tile @click='register' :loading='loading'
											:disabled='!valid || loading'>{{ applyButton }}
				</v-btn>
			</v-card-actions>
	...
	data() {
		return {
			...
			rules: {
				...
				confirm: v => (v && v === this.user.password) || 'Passwords do not match',
				email: v => (v && /.+@.+\..+/.test(v)) || 'E-mail must be valid',
				nameRules: [
					v => !!v || 'The field is required',
					v => (v && v.length >= 5) || 'Min 5 characters',
					v => (v && /^[a-zA-Z0-9-_]+$/g.test(v)) || 'Unsupported characters. Must be in range [a-zA-Z0-9-_]',
				],
			},
			user: {
				name: '',
