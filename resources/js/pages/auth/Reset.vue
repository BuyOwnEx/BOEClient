    ...
    <div class="layout-content ma-auto">
        <v-card>
            ...
            <v-card-actions class="pt-4 pl-6 pr-6 pb-4">
                <v-btn block color="primary" tile @click="set" :loading="loading" :disabled="!valid || loading">{{applyButton}}</v-btn>
            </v-card-actions>
        </v-card>
        <v-alert v-if="reset_alert"
                 dense
                 text
                 type="warning"
                 class="mt-4"
    ...
    export default {
        ...
        data() {
            return {
                ...
                formSubTitle: 'Please, store your password in a safe place and do not give it to other persons',
                applyButton: 'Set',
                show: false,
                show_confirm: false,
                rules: {
