    ...
    export default {
                    ...
                    yAxis: [
                        {
                            ...
                            title: {
                                text: this.$vuetify.lang.translator('trading.rate') + ', ' + this.market
                            },
                            height: '75%',
                            lineWidth: 1,
                            floor: 0,
                            ...
                            title: {
                                text: this.$vuetify.lang.translator('trading.volume') + ', ' + this.currency
                            },
                            top: '75%',
                            height: '25%',
                            offset: 35,
                    ...
                    series: [
                        {
                            type: 'candlestick',
                            id: 'main',
                            name: this.$vuetify.lang.translator('trading.rate') + ', ' + this.market,
                            data: [],
                            yAxis: 0,
                            xAxis: 0,
                            color: '#dd696b',
                            ...
                            tooltip: {
                                pointFormatter: function () {
                                    return '<span style="color:' + this.color + '">●</span> <b> ' + this.series.name + '</b><br/>Open: ' + BigNumber(this.open).toString() + '<br/>High: ' + BigNumber(this.high).toString() + '<br/>Low: ' + BigNumber(this.low).toString() + '<br/>Close: ' + BigNumber(this.close).toString() + '<br/>';
                                }
                            }
                        },
                        {
                            ...
                            id: 'volume',
                            linkedTo: 'main',
                            name: this.$vuetify.lang.translator('trading.volume') + ', ' + this.currency,
                            data: [],
                            yAxis: 1,
                            xAxis: 0
                        }
        ...
        watch: {
            ...
            pair(val) {
                ...
                candle.chart.series.forEach((item, i) => {
                    let id = _.get(item, 'options.id', undefined);
                    if (id === 'main') {
                        candle.chart.series[i].update({name: this.$vuetify.lang.translator('trading.rate') + ', ' + val[1]});
                    }
                    if (id === 'volume') {
                        candle.chart.series[i].update({name: this.$vuetify.lang.translator('trading.volume') + ', ' + val[0]});
                    }
                });
        ...
        created() {
            ...
            if ('localStorage' in window) {
                ...
                if(candlePeriod) {
                    let candlePeriodObject = _.find(this.options.rangeSelector.buttons, item => {
