	...
	<CommonDialog :confirm-color="getTypeColor" :header-color="getTypeColor" @confirm="confirm" :disabled="!isLogged || (isLogged && (!verifyStatus && !isNonKYCTradingAvailable))">
      ...
      <div v-else-if="isLogged && (!verifyStatus && !isNonKYCTradingAvailable)" class="mt-2 mb-2">
        <span class="red--text"><b>{{ $t('trading.forms.must_kyc') }}</b></span>
          ...
          <li v-if="isBuy && stopLoss">{{ $t('trading.forms.dialog.buy_cond_text') }} <b>{{ stopLoss }} {{ market }}</b> {{ $t('trading.forms.dialog.sl_order') }}</li>
          <li v-if="isBuy && takeProfit">{{ $t('trading.forms.dialog.buy_cond_text') }} <b>{{ takeProfit }} {{ market }}</b> {{ $t('trading.forms.dialog.tp_order') }}</li>
          <li v-if="isBuy && trailingStop">{{ $t('trading.forms.dialog.buy_cond_ts_text') }} <b>{{ trailingStop }}</b> {{ $t('trading.forms.dialog.points') }} {{ $t('trading.forms.dialog.ts_order') }}</li>

          <li v-if="isSell && stopLoss">{{ $t('trading.forms.dialog.sell_cond_text') }} <b>{{ stopLoss }} {{ market }}</b> {{ $t('trading.forms.dialog.sl_order') }}</li>
          <li v-if="isSell && takeProfit">{{ $t('trading.forms.dialog.sell_cond_text') }} <b>{{ takeProfit }} {{ market }}</b> {{ $t('trading.forms.dialog.tp_order') }}</li>
          <li v-if="isSell && trailingStop">{{ $t('trading.forms.dialog.sell_cond_ts_text') }} <b>{{ trailingStop }}</b> {{ $t('trading.forms.dialog.points') }} {{ $t('trading.forms.dialog.ts_order') }}</li>
				</ul>
...
export default {
    ...
    offerDays() {
      ...
      const anyParamExist = (this.stopLoss !== null && this.stopLoss !== '') || (this.takeProfit !== null && this.takeProfit !== '') || (this.trailingStop !== null && this.trailingStop !== '');
			return anyParamExist && this.isAdditionalParams;
		},
		isMultiplyAdditionalParams() {
			const isFirstAndSecond = (this.stopLoss !== null && this.stopLoss !== '') && (this.takeProfit !== null && this.takeProfit !== '');
			const isFirstAndThird = (this.stopLoss !== null && this.stopLoss !== '') && (this.trailingStop !== null && this.trailingStop !== '');
			const isSecondAndThird = (this.takeProfit !== null && this.takeProfit !== '') && (this.trailingStop !== null && this.trailingStop !== '');
			const isAll = (this.stopLoss !== null && this.stopLoss !== '') && (this.takeProfit !== null && this.takeProfit !== '') && (this.trailingStop !== null && this.trailingStop !== '');
