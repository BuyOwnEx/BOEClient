    ...
    <v-card>
        ...
        >
            ...
            <template v-slot:item.market="{ item }">
                <strong>{{ item.currency.toUpperCase() }}</strong><span class="market-name">/{{ item.market.toUpperCase() }}</span>
            </template>
            <template v-slot:item.side="{ item }">
                <strong :class="{'text-success':item.side, 'text-danger':!item.side}">
                    {{item.side ? $vuetify.lang.translator('trading.order.direction.buy') : $vuetify.lang.translator('trading.order.direction.sell') }}
                </strong>
            </template>
    ...
    export default {
        ...
        data() {
            return {
                ...
                tableCaption: this.$vuetify.lang.translator('trading.headers.own_history_deal_list') + ' ' +this.currency +'/'+ this.market,
                itemsPerPage: 5,
                headers: [
                    {
        ...
        computed: {
            ownDealList() {
                return this.showOtherPairs ? this.$store.state.user.deals : _.filter(this.$store.state.user.deals, {currency: this.currency.toUpperCase(), market: this.market.toUpperCase()});
            }
        },
        methods: {
            volume: function (item) {
                return BigNumber(item.price).times(BigNumber(item.size)).toString();
            },
            size: function (item) {
