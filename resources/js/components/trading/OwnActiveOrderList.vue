    ...
    <v-card>
        ...
        >
                    ...
                    <v-menu transition="slide-y-transition" bottom>
                        <template v-slot:activator="{ on, attrs }">
                            <v-btn tile outlined small color="#A6A6A6" v-bind="attrs" v-on="on">
                                {{$vuetify.lang.translator('trading.cancel')}} <v-icon right>mdi-chevron-down</v-icon>
                            </v-btn>
                        </template>
                        <v-list>
                            <v-list-item
            ...
            <template v-slot:item.action="{ item }">
                <v-btn tile outlined x-small color="#A6A6A6" dense @click="orderCancel(item)">
            ...
            <span v-if="item.side === false">
                <strong class="text-success">{{$vuetify.lang.translator('trading.order.direction.buy')}}</strong>
                <span class="order_type order_type_sl" v-if="item.type==='STOPLOSS'">SL</span>
                <span class="order_type order_type_tp" v-if="item.type==='TAKEPROFIT'">TP</span>
                <span class="order_type order_type_ts" v-if="item.type==='TRAILINGSTOP'">TS</span>
            </span>
                <span v-else>
                <strong class="text-danger">{{$vuetify.lang.translator('trading.order.direction.sell')}}</strong>
                <span class="order_type order_type_sl" v-if="item.type==='STOPLOSS'">SL</span>
                <span class="order_type order_type_tp" v-if="item.type==='TAKEPROFIT'">TP</span>
                <span class="order_type order_type_ts" v-if="item.type==='TRAILINGSTOP'">TS</span>
            </span>
            ...
            <template v-slot:item.status="{ item }">
                <span class="text-success" v-if="item.status==='accepted'">{{$vuetify.lang.translator('trading.order.status.accepted')}}</span>
                <span class="text-warning" v-else>{{$vuetify.lang.translator('trading.order.status.partiallyFilled')}}</span>
            </template>
        </v-data-table>
    </v-card>
</template>
    ...
    export default {
        ...
        data() {
            return {
                ...
                tableCaption: this.$vuetify.lang.translator('trading.headers.own_active_order_list') + ' ' +this.currency +'/'+ this.market,
                itemsPerPage: 5,
                headers: [
                    {
                    ...
                    { text: this.$vuetify.lang.translator('trading.amount'), value: 'size' },
                    { text: this.$vuetify.lang.translator('trading.rate'), value: 'price' },
                    { text: this.$vuetify.lang.translator('trading.volume'), value: 'volume', sortable: false },
                    { text: this.$vuetify.lang.translator('trading.filled'), value: 'percent' },
