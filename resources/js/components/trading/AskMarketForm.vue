<template>
    <form id="ask_market_form" @submit.prevent="sendAskMarket">
        ...
        <v-text-field
                ...
                hide-details
                height="28"
                :label="$vuetify.lang.translator('trading.order.direction.sell')"
                type="text"
        ...
        <v-text-field
                ...
                :hint="$vuetify.lang.translator('trading.order.pay_market_sell',[fee_visible]) + ' ' + market.toUpperCase()"
                persistent-hint
                type="text"
                @keydown="validateNumber($event)"
        >
        ...
        <div class="d-flex justify-end" v-if="marginTradingAvailable">
            <v-switch
                    v-model="useMargin"
                    class="mt-1"
                    hide-details
        ...
        <div class="box-footer">
            <v-btn text tile type="submit" height="32">{{$vuetify.lang.translator('trading.order.direction.sell')}}</v-btn>
...
<script>
    ...
    export default {
        ...
        computed: {
            ...
            bestBid() {
                let ticker = _.find(this.tickersFromStorage, item => item.currency.toUpperCase() === this.currency.toUpperCase() && item.market.toUpperCase() === this.market.toUpperCase());
                return ticker.bid;
            },
            markets() {
        ...
        methods: {
            ...
            updateVolume() {
                let price = BigNumber(this.bestBid).decimalPlaces(this.rateScale, 1).toString();
                this.volume = BigNumber(this.form.amount).times(BigNumber(price).times(BigNumber(1 - this.fee_market))).toString();
            },
            updateOffers() {
                if(this.useMargin)
                {
            ...
            updateAmount() {
                ...
                let newAmount = BigNumber(this.volume).div(BigNumber(price).times(BigNumber(1 - this.fee_market))).decimalPlaces(this.amountScale, 1).toString();
            ...
            sendAskMarket(event) {
                ...
                axios.post('/trader/ext/order/market', form)
                    .then((response) => {
            ...
            volume(newVolume, oldVolume) {
                if (this.isNumeric(newVolume) || newVolume === '') {
                    ...
                    if (rl.test(newVolume)) {
                        ...
                        if (this.bestBid !== 0 && this.volume !== '' && this.volume !== 0)
