<template>
    <form id="ask_limit_form" @submit.prevent="sendAskLimit">
        ...
        <v-text-field
                ...
                :hint="$vuetify.lang.translator('trading.order.pay_limit_sell',[fee_visible]) + ' ' + market.toUpperCase()"
                persistent-hint
                type="text"
                @keydown="validateNumber($event)"
        >
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            updateVolume() {
                this.volume = BigNumber(this.form.amount).times(BigNumber(this.form.rate).times(BigNumber(1 - this.fee_market))).toString();
            },
            updateOffers() {
                if(this.useMargin)
                {
            ...
            updateAmount() {
                ...
                let newAmount = BigNumber(this.volume).div(BigNumber(this.form.rate).times(BigNumber(1 - this.fee_market))).decimalPlaces(this.amountScale, 1).toString();
            ...
            getRateFromTicker() {
                ...
                this.form.rate = BigNumber(this.activeTicker.bid).dp(this.rateScale, 1).toString();
            }
        },
        watch: {
