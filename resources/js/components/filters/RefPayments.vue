    ...
    export default {
        ...
        methods: {
            makeFollowerSearch: async (value, self) => {
                ...
                await axios.get('/trader/ext/all_followers', {
                    params: {name: value}
                }).then(response => {
            ...
            setApplyEnable() {
                ...
                if(this.filters.dates[0]!==undefined && this.filters.dates[1]!==undefined)
                    this.disabled = false;
            },
            formatDate (dates) {
                if (!dates) return null;
                return `${dates[0]=== undefined ? 'YYYY-MM-DD': dates[0]} - ${dates[1] === undefined ? 'YYYY-MM-DD': dates[1]}`
            },
            setEnabled() {
                this.disabled = false;
            },
            ...
            resetFilter() {
                this.filters.dates = [moment().startOf('month').format('YYYY-MM-DD'),moment().format('YYYY-MM-DD')];
