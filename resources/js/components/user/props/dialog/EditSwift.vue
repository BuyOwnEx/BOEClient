...
export default {
  ...
  data() {
    return {
      valid: false,
      form: {
        id: this.swift_props.id,
        name: this.swift_props.name
      },
      errors: {
        name: [],
      }
  ...
  methods: {
    ...mapActions({
      editSwiftPropsNameStore: 'user/editSwiftPropsName',
    }),
    async edit() {
      ...
      try {
        ...
        const success = await this.editSwiftPropsNameStore(payload);
        if (success) {
