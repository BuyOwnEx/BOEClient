                ...
                <v-row class="mb-2 mt-1">
                  ...
                  <v-col cols="12" md="12" class="pt-0 pb-0 mt-2">
                    <v-text-field
                        ...
                        v-mask="'####################'"
                        :hint="$t('user.props.dialog.add.acc_hint')"
                        persistent-hint
                        required
                        dense
                    >
...
export default {
  ...
  methods: {
    ...mapActions({
      formAddRubPropsRequestStore: 'user/formAddRubPropsRequest',
      formAddRubPropsConfirmStore: 'user/formAddRubPropsConfirm',
    }),
    async sendConfirmation() {
      if (!this.valid) return;
      try {
        this.startLoading();
        await this.formAddRubPropsRequestStore(this.form);
        this.step++;
      } finally {
        this.stopLoading();
      }
    ...
    handleCodeInput(value) {
      if (value.length === 6 && this.emailCode) this.confirmAddRubProps();
    },
    async confirmAddRubProps() {
      ...
      try {
        ...
        const success = await this.formAddRubPropsConfirmStore(payload);
        if (success) {
          await this.$store.dispatch('user/getRubPropsFromServer');
        }
