...
export default {
  ...
  data() {
    return {
      valid: false,
      form: {
        id: this.rub_props.id,
        name: this.rub_props.name
      },
      errors: {
        name: [],
      }
  ...
  methods: {
    ...mapActions({
      editRubPropsNameStore: 'user/editRubPropsName',
    }),
    async edit() {
      ...
      try {
        ...
        const success = await this.editRubPropsNameStore(payload);
        if (success) {
