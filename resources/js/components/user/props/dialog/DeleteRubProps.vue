...
export default {
  ...
  methods: {
    ...mapActions({
      formDeleteRubPropsRequestStore: 'user/formDeleteRubPropsRequest',
      formDeleteRubPropsConfirmStore: 'user/formDeleteRubPropsConfirm',
    }),
    async sendConfirmation() {
      try {
        this.startLoading();
        await this.formDeleteRubPropsRequestStore(this.form);
        this.step++;
      } finally {
        this.stopLoading();
      }
    ...
    handleCodeInput(value) {
      if (value.length === 6 && this.emailCode) this.confirmDeleteRubProps();
    },
    async confirmDeleteRubProps() {
      ...
      try {
        ...
        const success = await this.formDeleteRubPropsConfirmStore(payload);
        if (success) {
