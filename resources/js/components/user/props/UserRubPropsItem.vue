  ...
  <v-card class="props-item">
    <v-card-title class="props-item__title" :class="getBackgroundClassColor">
      ...
      <div :class="[$vuetify.rtl ? 'mr-1' : 'ml-1']">
        <edit-props :rub_props="propItem" />
        <delete-rub-props :rub_props="propItem" :two-fa="twoFa"></delete-rub-props>
      </div>
    </v-card-title>
...
<script>
import EditProps from '@/components/user/props/dialog/EditRubPropsName.vue';
import DeleteRubProps from '@/components/user/props/dialog/DeleteRubProps.vue';
import CommonCopyLabel from '@/components/common/CommonCopyLabel.vue';
import formatDate from '@/mixins/format/formatDate';
export default {
  name: 'UserRubPropsItem',
  components: {
    EditProps,
    DeleteRubProps,
    CommonCopyLabel
  ...
  computed: {
    ...
    getBackgroundClassColor() {
      if (this.propItem.state === 'RP_CONFIRMED')
        return 'props-item__title--success';
      else if (this.propItem.state === 'RP_REJECTED')
        return 'props-item__title--error';
      else if (this.propItem.state === 'RP_ENTERED')
        return 'props-item__title--warning';
      else if (this.propItem.state === 'RP_DELETED')
